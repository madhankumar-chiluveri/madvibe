...
If Convex is configured with `CUSTOM_AUTH_SITE_URL=https://your-app.vercel.app`, the Next.js app now proxies `/api/auth/signin/*` and `/api/auth/callback/*` to `CONVEX_SITE_URL`, so both the Convex site callback and the Vercel app callback can be supported without 404s.
