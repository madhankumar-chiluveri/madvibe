...
export function BoardView({
  ...
  if (!groupProperty) {
    return (
      <div className="rounded-[24px] border border-foreground/8 bg-foreground/20 px-6 py-10 text-center text-sm text-muted-foreground">
  ...
  const renderCard = (row: any) => {
    ...
    return (
      <div
        key={row._id}
        className="rounded-[20px] border border-foreground/8 bg-card p-3 shadow-[0_12px_30px_rgba(0,0,0,0.24)] transition hover:border-foreground/12 hover:bg-accent"
      >
        <div className="mb-3 flex items-center gap-2">
          <p className="min-w-0 flex-1 text-sm font-medium text-foreground">{title || "Untitled"}</p>
        ...
        <div className="space-y-2">
          {visibleProperties.slice(0, 4).map((property) => (
            <div key={property.id} className="rounded-xl bg-foreground/[0.02] p-1">
  ...
  return (
    <div className="flex min-h-[420px] gap-4 overflow-x-auto pb-4 pt-2">
      {rows === undefined ? (
        <div className="rounded-[24px] border border-foreground/8 bg-foreground/20 px-6 py-10 text-sm text-muted-foreground">
          Loading board...
        </div>
      ) : columns.length === 0 ? (
        <div className="rounded-[24px] border border-foreground/8 bg-foreground/20 px-6 py-10 text-sm text-muted-foreground">
          Add options to <span className="text-foreground/80">{groupProperty.name}</span> to use Board view.
        </div>
      ) : (
        <>
          {(noGroupRows?.length ?? 0) > 0 && (
            <div className="w-[290px] shrink-0 rounded-[24px] border border-foreground/8 bg-foreground/20 p-3">
              <div className="mb-3 flex items-center justify-between px-1">
                <h3 className="text-sm font-medium text-foreground/80">No group</h3>
                <span className="text-xs text-muted-foreground">{noGroupRows?.length ?? 0}</span>
              </div>
          ...
          {columns.map((column) => {
            ...
            return (
              <div key={column.id} className="w-[290px] shrink-0 rounded-[24px] border border-foreground/8 bg-foreground/20 p-3">
                <div className="mb-3 flex items-center justify-between gap-3 px-1">
                  ...
                  {editable ? (
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 rounded-lg text-muted-foreground hover:bg-foreground/[0.06] hover:text-foreground"
                ...
                <div className="space-y-3 rounded-[18px] bg-background p-2">
                  {columnRows.length > 0 ? (
                    columnRows.map(renderCard)
                  ) : (
                    <div className="rounded-[14px] border border-dashed border-foreground/8 px-3 py-6 text-center text-xs text-muted-foreground/70">
